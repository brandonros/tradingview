    ...
    R: AsyncRead + Unpin,
{
    ...
    pub async fn read_message(&mut self) -> SimpleResult<Option<TradingViewMessageWrapper>> {
        loop {
    ...
    fn parse_message(&mut self) -> SimpleResult<Option<TradingViewMessageWrapper>> {
        if self.buffer.is_empty() {
            return Ok(None);
        }
